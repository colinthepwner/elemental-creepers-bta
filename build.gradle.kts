...
/**
 * Fails the build if any file the asset sidecar is supposed to supply is sitting in the source tree.
 *
 * This is the other half of the sidecar, and it is driven by the same manifest so the two cannot
 * drift: every path on the right-hand side of `asset-sidecar.properties` is a file this repository
 * must NOT contain, because the sidecar's whole purpose is to read that file out of a copy the
 * player supplies instead.
 *
 * Wired into `check` rather than left as a thing to remember, so a stray copy of the original mod's
 * skins cannot reach a public repository or a released jar by being forgotten about. Skipped
 * silently when Python is unavailable -- the guard is a safety net, not a build dependency.
 */
val verifyNoSidecarAssets by tasks.registering(Exec::class) {
	group = "verification"
	description = "Fails if art the asset sidecar should supply is present in the tree."
	isIgnoreExitValue = false
	commandLine("python", "tools/strip_sidecar_assets.py", "--check")
	// A machine without Python still builds; it just does not get the check.
	onlyIf {
		try {
			ProcessBuilder("python", "--version").start().waitFor() == 0
		} catch (e: Exception) {
			logger.warn("Python not found; skipping the sidecar-asset check.")
			false
		}
	}
}

tasks.named("check") { dependsOn(verifyNoSidecarAssets) }
