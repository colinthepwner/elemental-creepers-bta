...
public final class GhostShockwave {

	private GhostShockwave() {}

	public static void detonate(@NotNull World world, @Nullable Entity exploder,
		...
		for (Entity target : targets) {
			...
			target.hurt(exploder, (int) ((force * force + force) / 2.0D * 8.0D * size + 1.0D),
				DamageType.BLAST);

			target.xd += dirX * force;
			target.yd += dirY * force;
			target.zd += dirZ * force;
		}

		ECEffects.explosionSound(world, x, y, z);
		if (exploder != null) {
			ECEffects.explosionPoof(world, exploder);
		}
	}
}
